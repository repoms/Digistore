...
<template>
    <div class="indicator w-full">
        ...
        <span class="indicator-item">
            <button @click="RemoveInvoice" class="btn btn-xs btn-circle bg-error border-none text-white transition ease-in-out hover:scale-125">
        ...
        <div class="card card-compact shadow-lg border">
            ...
            <div class="card-body flex-row justify-between">
                ...
                <div class="dropdown dropdown-end w-full text-end">
                    ...
                    <ul tabindex="0" class="dropdown-content menu menu-compact p-2 shadow w-full text-primary-content bg-neutral rounded-lg">
                        <li v-for="status in GetAllStatus" :class="status.color" class="hover:bg-neutral" >
                            <a @click="changeStatus(status.statusCode)">
                                <font-awesome-icon :icon="['fas', status.icon]" :class="status.flip ? '-scale-x-100' : ''" />
                                {{ status.status }}
                            </a>
                        </li>
                    </ul>
...
export default {
    ...
    methods: {
        ...
        async RemoveInvoice() {
            const Result = await apiHandler.PRE_API.RemoveInvoice(this.invoice.Guid)
            console.log(Result)
