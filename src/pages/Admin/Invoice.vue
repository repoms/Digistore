<template>
	<div class="card min-w-min p-6 shadow-xl bg-base-100">
		<!-- Title -->
		<div class="text-xl font-semibold">Semua Pesanan</div>

		<div class="divider mt-2"></div>

		<!-- Content -->
		<div class="h-full w-full pb-6 grid sm:grid-cols-1 lg:grid-cols-3 gap-3 gap-y-5">
			<InvoiceCard v-for="invoice in invoices" :invoice="invoice" />
		</div>
	</div>
</template>
...
export default {
	...
	async mounted() {
		const Result = await apiHandler.PRE_API.GetAllInvoice()
		console.log(Result)
