...
<template>
    <div class="card card-compact shadow-lg border">
        ...
        <div class="card-body flex-row justify-between">
            ...
            <div class="dropdown dropdown-end w-full text-end" v-if="GetStatus.statusCode == 0">
                ...
                <ul tabindex="0" class="dropdown-content menu menu-compact p-2 shadow w-full text-primary-content bg-neutral rounded-lg">
                    <li :class="GetProcessStatus.color" class="hover:bg-neutral" >
                        <a @click="changeStatus(GetProcessStatus.statusCode)">
                            <font-awesome-icon :icon="['fas', GetProcessStatus.icon]" :class="GetProcessStatus.flip ? '-scale-x-100' : ''" />
            ...
            <div class="dropdown dropdown-end w-full text-end" v-else-if="GetStatus.statusCode == 1">
                <label tabindex="0" :class="GetStatus.color" class="btn btn-sm normal-case gap-3 border-0">
                    <font-awesome-icon :icon="['fas', GetStatus.icon]" :class="GetStatus.flip ? '-scale-x-100' : ''" />
                    {{ GetStatus.status }}
                    <font-awesome-icon icon="fa-solid fa-chevron-down" />
                ...
                <ul tabindex="0" class="dropdown-content menu menu-compact p-2 shadow w-full text-primary-content bg-neutral rounded-lg">
                    <li :class="GetDeliveryStatus.color" class="hover:bg-neutral" >
                        <a @click="changeStatus(GetDeliveryStatus.statusCode)">
                            <font-awesome-icon :icon="['fas', GetDeliveryStatus.icon]" :class="GetDeliveryStatus.flip ? '-scale-x-100' : ''" />
            ...
            <div class="dropdown dropdown-end w-full text-end" v-else-if="GetStatus.statusCode == 6">
                <label tabindex="0" :class="GetStatus.color" class="btn btn-sm normal-case gap-3 border-0">
                    <font-awesome-icon :icon="['fas', GetStatus.icon]" :class="GetStatus.flip ? '-scale-x-100' : ''" />
                    {{ GetStatus.status }}
                    <font-awesome-icon icon="fa-solid fa-chevron-down" />
                ...
                <ul tabindex="0" class="dropdown-content menu menu-compact p-2 shadow w-full text-primary-content bg-neutral rounded-lg">
                    <li :class="GetReturnedStatus.color" class="hover:bg-neutral" >
                        <a @click="statusDikirimBalik()">
                            <font-awesome-icon :icon="['fas', GetReturnedStatus.icon]" :class="GetReturnedStatus.flip ? '-scale-x-100' : ''" />
                            {{ GetReturnedStatus.status }}
                        </a>
                    </li>
                </ul>
            </div>
...
export default {
    ...
    computed: {
        ...
        GetProcessStatus() {
            return utils.StatusCode[1]
        },
        GetDeliveryStatus() {
            return utils.StatusCode[2]
        },
        GetReturnedStatus() {
            return utils.StatusCode[0]
        }
    },
    methods: {
        ...
        async statusDikirimBalik() {
            const Result = await apiHandler.PRE_API.InvoiceStatusReturn(this.invoice.Guid)
            console.log(Result)
